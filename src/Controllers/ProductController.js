...
module.exports = {
    async save(req,res){
        ...
        if(hasNull(req.body, ['id_brand', 'name', 'price', 'category'])){
            if(req.file)
                deleteFile(req.file.key);
            return res.status(403).send({ msg: 'missing required data' });
        }

        const {name, description, price, image_uri, category, id_brand} = req.body;
        ...
        try {
            ...
            if(req.file){
                ...
                product = await Product.create({name, description, price, category, id_brand})
    ...
    async list(req,res){
        ...
        if(hasNull(req.query, ['limit', 'page']))
            return res.status(404).send({msg: 'missing required data'});
